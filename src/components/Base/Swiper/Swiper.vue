...
export default {
  ...
  methods: {
    ...
    touchMove(e) {
      ...
      // 计算当前的元素位置
      const currentPosition = -this.currentIndex * this.slideWidth
      // 计算拖动后的位置
      const movePosition = currentPosition + distance
    ...
    touchEnd(e) {
      ...
      // 计算拖动到多少滑动到下一张
      const isSlide = Math.abs(distance) >= this.slideWidth * this.moveRadio
    ...
    handleDom() {
      ...
      if (this.slideCount > 0) {
        const first = children[0].$el
        // this.swiper = this.$el.firstElementChild // 获取拖动的swiper元素
        // console.log(this.swiper)
        this.slideWidth = first.offsetWidth // 获取一个滑块的宽度
        const cloneFirst = first.cloneNode(true) // 克隆第一个滑块
        const cloneLast = children[this.slideCount - 1].$el.cloneNode(true) // 克隆最后一个滑块
